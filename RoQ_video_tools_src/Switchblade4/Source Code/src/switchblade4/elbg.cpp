...
typedef struct{
    ...
    terVector<cell *> cells;
    terVector<int> utility;
    terVector<int> utility_inc;
                  ...
                  AVRandomState *rand_state)
{
    ...
    if (numpoints > 24*numCB) {
        ...
        terVector<int> temp_points;
		temp_points.Alloc(dim*(numpoints/8));
                ...
                AVRandomState *rand_state)
{
    ...
    cell *free_cells;
    terVector<int> dist_cb;
    terVector<int> size_part;
    terVector<cell> list_buffer;

	dist_cb.Alloc(numpoints);
	size_part.Alloc(numCB);
	list_buffer.Alloc(numpoints);
    ...
    elbg->cells.Alloc(numCB);
    elbg->utility.Alloc(numCB);
    elbg->nearest_cb = closest_cb;
    elbg->points = points;
    elbg->utility_inc.Alloc(numCB);
